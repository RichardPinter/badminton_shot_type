...
def get_stroke_types(side='Both', pad_to_same_len=False):
    ...
    if pad_to_same_len:
        ...
        match side:
            case 'Both':
                class_ls = ['Top_'+s+' '*3 for s in class_ls] + ['Bottom_'+s for s in class_ls]
            case 'Top':
                class_ls = ['Top_'+s+' '*3 for s in class_ls]
            case 'Bottom':
                class_ls = ['Bottom_'+s for s in class_ls]

        return class_ls

    match side:
        case 'Both':
            class_ls = ['Top_'+s for s in class_ls] + ['Bottom_'+s for s in class_ls]
        case 'Top':
            class_ls = ['Top_'+s for s in class_ls]
        case 'Bottom':
            class_ls = ['Bottom_'+s for s in class_ls]
...
def get_frontier_categories():
    '''Returns the 6 frontier categories (excluding unknown).'''
    return ['net', 'smash', 'lob', 'clear', 'drive', 'drop']


def get_stroke_to_frontier_mapping(csv_path='ShuttleSetMapping.csv'):
    '''
    Reads the CSV mapping and returns a dict mapping English stroke names to frontier class indices.
    Excludes strokes that map to 'unknown'.

    Returns:
        dict: {stroke_name: frontier_class_index}
    '''
    df = pd.read_csv(csv_path)
    frontiers = get_frontier_categories()

    mapping = {}
    for _, row in df.iterrows():
        english_name = row['English']
        frontier = row['EnglishFrontiers']

        # Skip strokes that map to 'unknown'
        if frontier == 'unknown':
            continue

        if frontier in frontiers:
            mapping[english_name] = frontiers.index(frontier)

    # Add wrist_smash mapping (not in CSV, maps to smash)
    mapping['wrist_smash'] = frontiers.index('smash')
...
def prepare_npy_collated_bottom_frontier_loaders(
    root_dir: Path,
    pose_style='Jn2B',
    batch_size=128,
    use_cuda=True,
    ...
    '''
    Prepares dataloaders for Bottom player only with frontier class labels (6 classes).
    Filters out 'none' stroke type and converts stroke labels to frontier labels.
    Note: Uses test set as validation set since val set is not processed.
    '''
    from dataset import Dataset_npy_collated_one_side

    # Load Bottom-only data with 19 stroke classes
    train_set = Dataset_npy_collated_one_side(root_dir, 'train', pose_style, use_top_side=False)
    val_set = Dataset_npy_collated_one_side(root_dir, 'test', pose_style, use_top_side=False)  # Use test as val
    test_set = Dataset_npy_collated_one_side(root_dir, 'test', pose_style, use_top_side=False)

    # Get stroke to frontier mapping
    stroke_to_frontier = get_stroke_to_frontier_mapping()
    stroke_types = get_stroke_types(side='Bottom')

    # Create mapping from stroke label (0-18) to frontier label (0-5), -1 for 'none'
    label_mapping = {}
    for stroke_idx, stroke_name in enumerate(stroke_types):
        # Remove 'Bottom_' prefix to get base stroke name
        base_stroke = stroke_name.replace('Bottom_', '')
        if base_stroke in stroke_to_frontier:
            label_mapping[stroke_idx] = stroke_to_frontier[base_stroke]
        else:
            label_mapping[stroke_idx] = -1  # Mark 'none' for filtering

    # Filter and convert labels for each dataset
    for dataset in [train_set, val_set, test_set]:
        # Create mask for valid samples (exclude 'none')
        valid_mask = np.array([label_mapping.get(label, -1) != -1 for label in dataset.labels])

        # Filter data
        dataset.human_pose = dataset.human_pose[valid_mask]
        dataset.pos = dataset.pos[valid_mask]
        dataset.shuttle = dataset.shuttle[valid_mask]
        dataset.videos_len = dataset.videos_len[valid_mask]

        # Convert labels to frontier labels
        dataset.labels = np.array([label_mapping[label] for label in dataset.labels[valid_mask]])
