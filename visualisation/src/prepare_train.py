...
def to_court_coordinate(
    ...
    '''
    if vid not in res_df.index:
        raise KeyError(f"Video ID {vid} not found in resolution data")
...
def detect_shuttlecock_by_TrackNetV3_with_attension(
    ...
    video_path: Path,
    save_dir: Path,
    model_folder=Path("/home/richard/Desktop/Projects/Personal/UNE/BST-Badminton-Stroke-type-Transformer/badminton-stroke-classifier/visualisation/tracknet"),
):
    '''
    ...
    # Add parent directory to Python path so tracknet module can be imported
    tracknet_parent = model_folder.parent

    env = os.environ.copy()
    if 'PYTHONPATH' in env:
        env['PYTHONPATH'] = f"{tracknet_parent}:{env['PYTHONPATH']}"
    else:
    ...
    r = subprocess.run(process_args, env=env)
...
def prepare_2d_dataset_npy_from_raw_video(
    ...
    for video_path in all_mp4_paths:
        # Check if video ID exists in resolution data
        vid = int(video_path.name.split('_', 1)[0])
        if vid not in resolution_df.index:
            print(f"Skipping video {video_path.name}: ID {vid} not found in resolution data")
            pbar.update()
            continue
...
def collate_single_video(
    ...
    """
    Process a single video's raw .npy files into collated format for BSTM evaluation.

    This is a single-video version of collate_npy() that processes one video at a time
    instead of batching an entire dataset.

    Args:
        joints_path: Path to _joints.npy file (t, 2, J, d)
        pos_path: Path to _pos.npy file (t, 2, xy)
        shuttle_path: Path to _shuttle.npy file (t, xy)
        output_dir: Directory where output files will be saved
        seq_len: Target sequence length (default: 100)
        label: Stroke type label (default: 0)
    ...
    shuttle = np.load(str(shuttle_path))

    # Get bone pairs for COCO skeleton
    bone_pairs = get_bone_pairs(skeleton_format='coco')

    # Process the video
    J_only, JnB_interp, JnB_bone, Jn2B, pos_out, shuttle_out, video_len = \
    ...
    J_only = np.expand_dims(J_only, axis=0)
    JnB_interp = np.expand_dims(JnB_interp, axis=0)
    JnB_bone = np.expand_dims(JnB_bone, axis=0)
    Jn2B = np.expand_dims(Jn2B, axis=0)
    pos_out = np.expand_dims(pos_out, axis=0)
    shuttle_out = np.expand_dims(shuttle_out, axis=0)
    videos_len = np.array([video_len], dtype=np.int32)
    labels = np.array([label], dtype=np.int64)

    # Save all outputs
    output_files = {
        'J_only': output_dir / 'J_only.npy',
        'JnB_interp': output_dir / 'JnB_interp.npy',
        'JnB_bone': output_dir / 'JnB_bone.npy',
        'Jn2B': output_dir / 'Jn2B.npy',
        'pos': output_dir / 'pos.npy',
        'shuttle': output_dir / 'shuttle.npy',
        'videos_len': output_dir / 'videos_len.npy',
        'labels': output_dir / 'labels.npy'
    }

    np.save(str(output_files['J_only']), J_only)
    np.save(str(output_files['JnB_interp']), JnB_interp)
    np.save(str(output_files['JnB_bone']), JnB_bone)
    np.save(str(output_files['Jn2B']), Jn2B)
    np.save(str(output_files['pos']), pos_out)
    np.save(str(output_files['shuttle']), shuttle_out)
    np.save(str(output_files['videos_len']), videos_len)
    np.save(str(output_files['labels']), labels)
...
if __name__ == '__main__':
    my_dataset_folder = Path("../ShuttleSet/shuttle_set_between_2_hits_with_max_limits")
    ...
    homo_df = pd.read_csv("../ShuttleSet/set/homography.csv").set_index('id')
    resolution_df = pd.read_csv("../ShuttleSet/my_raw_video_resolution.csv").set_index('id')
