...
from match_loader import Match, PlayerSet, SetShot, ShotType, MatchUtils, ShotUtils
...
class ShotClassifier:
    ...
    def __init__(self, logger: logging.Logger, shot_types: List[ShotType], model_persist_path: str, config_path: str = 'config.json'):
    ...
    def predict(self, modelPath:str, set_shots: List[SetShot]) -> List[str]:
        # Load model with custom objects to handle Keras 2 -> Keras 3 compatibility
        try:
            model = load_model(modelPath, compile=False)
        except Exception as e:
            self.logger.warning(f"Failed to load model with default settings: {e}")
            self.logger.info("Attempting to load with custom_objects workaround...")
            # Try loading without time_major parameter (Keras 3 doesn't support it)
            from tensorflow.keras.layers import LSTM as LSTM_Layer
            model = load_model(modelPath, compile=False, custom_objects={'LSTM': LSTM_Layer})
